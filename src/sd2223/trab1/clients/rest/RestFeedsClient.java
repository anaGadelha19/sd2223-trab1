...
public class RestFeedsClient extends RestClient implements Feeds {
    ...
    private Result<Long> clt_postMessage(String user, String pwd, Message msg) {
        Response r = target.path(user)
                .queryParam(UsersService.PWD, pwd).request() // UsersService or FeedsService??
    ...
    private Result<Void> clt_removeFromPersonalFeed(String user, long mid, String pwd) {
        ...
        if (r.getStatus() == Status.OK.getStatusCode() && r.hasEntity()) {
            System.out.println("Message removed from personal Feed");
            return Result.ok();
        } else {
            System.out.println("Error, HTTP error status: " + r.getStatus());
            return Result.error(getErrorCodeFrom(r.getStatus()));
    ...
    private Result<Void> clt_removeDeletedUserFeed(User user) {
    ...
    public Result<Void> removeDeletedUserFeed(User user) {
        ...
        return super.reTry(() -> clt_removeDeletedUserFeed(user));
    }
